class Stat < ActiveRecord::Base
  ...
  def shifts(reset=false)
    if reset || @shifts.nil? # recalculate if reset is on or @shifts not calculated yet
      @shifts = find_shifts
      t1 = filter_by_location_groups(@shifts)
  ...
  def location_group_list
    shifts.collect { |sh| sh.location.location_group }.uniq
  end
  ...
  def self.split_to_location_groups(shift_list)
    (shift_list||[]).group_by {|s| s.location.location_group_id}
  end
  ...
  def self.filter_missed(shift_list)
    (shift_list||[]).select { |e| e.shift_report.nil? }
  end
  ...
  def self.actual_vs_scheduled(shift_list)
    ...
    (shift_list||[]).each do |sh|
      if sh.scheduled?
        ...
        covered += (sh.shift_report.end - sh.shift_report.start) if sh.shift_report && sh.shift_report.end
      end
    end
    if total.zero?
      0
  ...
  def filter_by_departments(shift_list)
    unless department_ids.blank?
      ...
      filtered = shift_list.select { |s| id_list.include?(s.location.location_group.department_id) }
    else
      shift_list
    end
  end
  ...
  def find_time_slots
    temp = TimeSlot.all_between(start, stop)
  ...
  def find_shifts
    Shift.all_between(start, stop).find(:all, :conditions => conditions, :include => {:location => :location_group})
  end
